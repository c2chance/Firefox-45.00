function s_initial_beacon() {
    s_domain_standards(s);
    s.t();
    typeof s_poc != "undefined" && s_account == "myfamilyancestry" && s_poc.t()
}

function s_fallback(s) {
    var now = new Date, track_dls, track_externals, internal_filters, leave_query;
    ...
    s.linkTrackVars = s.linkTrackVars ? s.linkTrackVars : "None";
    s.charSet = "UTF-8";
    s.cookieDomainPeriods = document.location.hostname.length - document.location.hostname.replace(/\./g, "").length;
    typeof s.pageName == "undefined" && (s.pageName = "none : none : none : needs valid page name");
    s.pageName == "ancestry us : dna : offers : generic offer" && (s.pageName = s.pageName + "-" + s.getQueryParam("code"));
    s.pageName == "ancestry us : secure : signup : register" && (s.getQueryParam("cp") == "1" && (s.pageName = s.pageName + "-User Password"),
    s.getQueryParam("cp") == "0" && (s.pageName = s.pageName + "-System Password"));
    s.pageName = s.pageName.toLowerCase();
    s.pageName.length - s.pageName.replace(/ : /g, "").length == 9 && (s.hier1 = s.pageName.replace(/ : /g, ":"),
    s.channel = s.pageName.replace(/^.*? : (.*?) : .*$/, "$1"),
    s.prop42 = s.pageName.replace(/^(.*? : .*?) : .*$/, "$1"),
    s.prop43 = s.pageName.replace(/^(.*? : .*?.*? : .*?) : .*$/, "$1"),
    s.eVar42 = s.pageName.replace(/^(.*? : .*?) : .*$/, "$1"),
    s.eVar43 = s.pageName.replace(/^(.*? : .*?.*? : .*?) : .*$/, "$1"));
    (typeof s.prop1 != "undefined" || s.prop1 == "") && s.prop1.indexOf(":") == -1 && (s.prop1 = "");
    s.pageName.indexOf(": search : form : category") != -1 && (s.prop22 = s.getQueryParam("cat") + ":n:category",
    s.eVar22 = "D=c22");
    s.prop38 || (s.prop38 = sc_getRegion(),
    s.eVar38 = s.prop38);
    s.prop41 = s.getPreviousValue(s.pageName, "gpv_pn");
    s.prop44 = dateFormat(now);
    s.prop45 = stripParameters();
    s.eVar41 && (s.prop50 = "D=v41");
    s.campaign && (s.prop53 = "D=v0",
    s.eVar62 = "D=v0");
    s.eVar4 && (s.prop54 = "D=v4");
    s.prop65 || (s.prop65 = s.getQueryParam("cat"));
    s.getQueryParam("gl") && s.pageName == "ancestry us : search : results" && (s.prop70 = "search category:" + s.getQueryParam("gl"));
    s.prop23 && (s.eVar23 = "D=c23");
    s.eVar41 = s.pageName;
    s.eVar44 = dateFormat(now);
    s.pageName.indexOf(": offer :") != -1 && (s.prop23 == "Registrant" || s.prop23 == "NRVisitor") && s.pageName.indexOf("upgrade") == -1 && (s.eVar61 = "+1");
    s.getQueryParam("survsel") && (s.eVar70 = "Radio Survey:" + s.getQueryParam("survsel"));
    s.getQueryParam("o_sch") == "Android App" && (s.sa("ancestrymobileapps"),
    keepVars("pageName,events,purchaseID,products,c11,c23,v23,c44,v44,c45"));
    s.trackDownloadLinks = track_dls ? track_dls : !0;
    s.trackExternalLinks = track_externals ? track_externals : !1;
    s.trackInternalFilters = internal_filters ? internal_filters : "javascript:,.ancestry.";
    s.linkLeaveQueryString = leave_query ? leave_query : !1
}

function sc_getRegion() {
    for (mc = document.getElementsByTagName("meta"),
    i = 0; i < mc.length; i++)
        if (na = mc[i].name.search(/geo.placename/),
        na != -1) {
            var r = mc[i].content;
            return r == "USA" ? "US" : r == "UK" ? "UK" : r == "Australia" ? "AU" : r == "France" ? "FR" : r == "Sweden" ? "SE" : r == "Canada" ? "CA" : r == "Italy" ? "IT" : r
        }
}

function sc_getKeyword() {
    var result = "", i = 0, url = document.referrer, qs, n, nLength, nk;
    if ((url.indexOf(".google") != -1 || url.indexOf(".yahoo") != -1 || url.indexOf(".bing") != -1 || url.indexOf(".aol") != -1 || url.indexOf(".mywebsearch") != -1 || url.indexOf(".comcast") != -1 || url.indexOf(".search-results") != -1 || url.indexOf(".dogpile") != -1 || url.indexOf(".baidu") != -1 || url.indexOf(".daum") != -1 || url.indexOf(".webalta") != -1 || url.indexOf(".excite") != -1 || url.indexOf(".ask") != -1 || url.indexOf(".virtilio") != -1 || url.indexOf(".searchalot") != -1) && (qs = url.split("?"),
    qs.length <= 1 && (qs = url.split("#")),
    qs.length > 1))
        for (n = qs[1].split("&"),
        nLength = n.length; i < n.length; )
            nk = n[i].split("="),
            nk[1] = nk[1].replace(/\+/g, " "),
            nk[1] = nk[1].replace(/"/g, ""),
            nk[1] = nk[1].replace(/%22/g, '"'),
            nk[1] = nk[1].replace(/%2B/g, " "),
            (url.indexOf(".google") != -1 || url.indexOf(".bing") != -1 || url.indexOf(".aol") != -1 || url.indexOf(".comcast") != -1 || url.indexOf(".search-results") != -1 || url.indexOf(".dogpile") != -1 || url.indexOf(".daum") != -1 || url.indexOf(".webalta") != -1 || url.indexOf(".excite") != -1 || url.indexOf(".searchalot") != -1 || url.indexOf(".ask.") != -1) && nk[0] == "q" && (result = nk[1]),
            url.indexOf(".yahoo") != -1 && nk[0] == "p" && (result = nk[1]),
            url.indexOf(".baidu") != -1 && nk[0] == "wd" && (result = nk[1]),
            (url.indexOf(".mywebsearch") != -1 || url.indexOf(".myway") != -1) && nk[0] == "searchfor" && (result = nk[1]),
            url.indexOf(".virtilio") != -1 && nk[0] == "qs" && (result = nk[1]),
            i++;
    return result = unescape(result),
    result.toLowerCase()
}

function sc_keepVars(kVar) {
    for (var i = 0; i < 100; i++)
        kVar.match(new RegExp("\\bc" + i + "\\b","g")) || (s["prop" + i] = ""),
        kVar.match(new RegExp("\\bv" + i + "\\b","g")) || (s["eVar" + i] = ""),
        i <= 5 && (kVar.match(new RegExp("\\bh" + i + "\\b","g")) || (s["hier" + i] = ""));
    for (svarArr = ["pageName", "channel", "products", "events", "campaign", "purchaseID", "state", "zip", "server", "linkName"],
    i = 0; i < svarArr.length; i++)
        kVar.match(new RegExp("\\b" + svarArr[i] + "\\b","g")) || (s[svarArr[i]] = "")
}

function sc_npv(v, va, d) {
    var tD, cD, vAr, vaAr, i;
    if (s.linkTrackVars = "",
    s.linkTrackEvents = "",
    tD = new Date,
    cD = new Date(d),
    tD < cD || d == 0) {
        ...
        i = 0; i < vAr.length; i++)
            s[vAr[i]] = vaAr[i],
            s[vAr[i]].match(/event/g) ? (s.linkTrackEvents += vaAr[i] + ",",
            s.linkTrackVars += "events,") : s.linkTrackVars += vAr[i] + ",";
        s.linkTrackVars.substring(s.linkTrackVars.length - 1, s.linkTrackVars.length) == "," && (s.linkTrackVars = s.linkTrackVars.substring(0, s.linkTrackVars.length - 1));
        s.linkTrackEvents.substring(s.linkTrackEvents.length - 1, s.linkTrackEvents.length) == "," && (s.linkTrackEvents = s.linkTrackEvents.substring(0, s.linkTrackEvents.length - 1));
...
function s_gi(un, pg, ss) {
    ...
    l))
        for (j = 0; j < 2; j++)
            for (i = 0; i < l.length; i++)
                if (s = l[i],
                x = s._c,
                (!x || x == "s_c" || j > 0 && x == "s_l") && (s.oun == un || s.fs && s.sa && s.fs(s.oun, un))) {
                    if (s.sa && s.sa(un),
                    x == "s_c")
                        return s
                } else
                    s = 0;
    return w.s_an = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz",
    ...
    c = s_d(c),
    e > 0 ? (a = parseInt(i = v.substring(e + 5)),
    a > 3 && (a = parseFloat(i))) : a = m > 0 ? parseFloat(u.substring(m + 10)) : parseFloat(v),
    (a < 5 || v.indexOf("Opera") >= 0 || u.indexOf("Opera") >= 0) && (c = s_ft(c)),
    s || (s = {},
    w.s_c_in || (w.s_c_il = [],
    w.s_c_in = 0),
    s._il = w.s_c_il,
...
var s, s_code, s_objectID;
s_account = s_account ? s_account : "ancestryerror";
s = s ? s : s_gi(s_account, 1);
