...
$trees.PedigreeObject.prototype = {
    ...
    discoverExistingExpansions: function () {
        ...
        expansionIdArray;
        if (obj != null) {
            if (pidListAttr = obj.getAttribute('T:PidList'), pidListAttr) for (expansionPidArray = pidListAttr.split(','), i = 0; i < expansionPidArray.length; i++) this.expansionPids.push(expansionPidArray[i]);
            if (nodeIdListAttr = obj.getAttribute('T:NodeIdList'), nodeIdListAttr) for (expansionIdArray = nodeIdListAttr.split(','), i = 0; i < expansionIdArray.length; i++) this.expansionNodeIds.push(expansionIdArray[i])
        }
    }
};
