(function($) {
    ...
    var shared = {
        ...
        timer: function(callback, delay) {
            var timerId, remaining = delay, start, state = "stopped", obj = {
                pause: function() {
                    state === "play" && (clearTimeout(timerId),
                    remaining -= new Date - start,
                    state = "pause")
                },
                stop: function() {
                    state !== "stopped" && (clearTimeout(timerId),
                    remaining = delay,
                    state = "stopped")
                },
                play: function() {
                    state !== "play" && (start = new Date,
      ...
      , alert = {
        create: function($element, options) {
            ...
            instance.config = $.extend({}, this.staticProperties.defaults, instance._getOptionsByClass(), options),
            instance.uniqueId = $element.attr("id") || shared.getRandomNumber(),
            instance.shouldAnimate = $("body").find(instance.$element).length && !shared.visible($element[0]) || !instance.config.open,
            instance.hasFocusableChildren = instance._hasTabbableElements(),
            instance.$element.attr(instance._getAttributes()),
            instance._initClasses(),
            instance.isOpen = !1,
            instance.bottomMargin = instance._getBottomMargin(),
            instance.config.display === "notification")
                instance.shouldAnimate = !0,
                instance._addOrRemoveNotification(),
                instance.config.closeable = !0,
                instance.config.removable = !0;
            ...
            return instance.config.open && instance.open(),
            instance.config.closeable && instance._createCloseButton(),
            instance.$element.data("alertInstance", instance),
            instance
        },
        proto: {
            ...
            center: function() {
                ...
                width = Math.ceil(this.$element[0].getBoundingClientRect().width),
                this.$element.css({
                    display: "",
            ...
            close: function() {
                ...
                this.isOpen && (this.$element.trigger("closing.alert"),
                marginShouldAnimate = this.$element.hasClass("alertSitewide") || this.$element.is(":first-child"),
                this.$element.addClass("alertHiding").css("margin-bottom", this.bottomMargin - this.$element.outerHeight(marginShouldAnimate)),
                setTimeout(function() {
            ...
            _createCloseButton: function() {
                var $closeBtn = this.$element.find(".alertCloseBtn")
                  , self = this;
                $closeBtn.length ? $closeBtn.addClass("closeBtn").empty() : ($closeBtn = $('<button class="closeBtn alertCloseBtn" type="button" title="' + shared.localize(alert.staticProperties.langSets, "closeBtn") + '"><\/button>'),
            ...
            _getAttributes: function() {
                ...
                return this.$element.attr("role") || (this.hasFocusableChildren ? (attr.role = "alertdialog",
                attr.tabindex = this.$element.attr("tabindex") || "-1") : attr.role = this.config.type === "warning" || this.config.type === "success" ? "alert" : "status"),
                this.$element.attr("aria-label") || (attr["aria-label"] = this.$element.contents().first().text()),
                attr
            },
            ...
            _getOptionsByClass: function() {
                var options = {};
                return this.$element.hasClass("alertSuccess") ? options.type = "success" : (this.$element.hasClass("alertInfo") || this.$element.hasClass("alertNote")) && (options.type = "info"),
                this.$element.hasClass("alertSitewide") ? options.display = "sitewide" : this.$element.hasClass("alertNotification") ? options.display = "notification" : this.$element.hasClass("alertSection") ? options.display = "section" : this.$element.hasClass("alertOverlay") && (options.display = "overlay"),
                this.$element.hasClass("alertSlim") && (options.size = "slim"),
                this.$element.hasClass("alertNoIcon") && (options.noIcon = !0),
                this.$element.hasClass("alertHidden") && (options.open = !1),
                this.$element.hasClass("alertCloseable") && (options.closeable = !0),
                options
            },
            ...
            open: function() {
                ...
                this.isOpen || (this.$element.trigger("opening.alert"),
                this.shouldAnimate ? (marginToHideAlert = this.$element.removeClass("alertHidden").outerHeight(!1),
                this.$element.addClass("alertNoTransition").css("margin-bottom", this.bottomMargin - marginToHideAlert),
                setTimeout(function() {
            ...
            _opened: function() {
                var self = this;
                if (this.hasFocusableChildren && this.$element.focus(),
    ...
    if (doubleIncluded) {
        ...
        if (shared.warn("ERROR: You are including multiple versions of alert.js"),
    ...
    $.fn.alert = function(options) {
        ...
        return this.each(function() {
            var self = $(this), instance = self.data("alertInstance"), methodValue;
            if (isMethodCall) {
                if (!instance)
                    return shared.warn('Cannot call methods on alert prior to initialization; attempted to call method "' + options + '".'),
                    !1;
                if (!$.isFunction(instance[options]) || options.indexOf("_") === 0)
                    return shared.warn('No such method "' + options + '" for alert.'),
                    !1;
