...
YAHOO.widget.DS_XHR = function (B, A, D) {
    ...
    return C._aDeprecatedSchema = A, C
};
...
YAHOO.widget.AutoComplete = function (G, B, J, C) {
    var D, K, E, A, I, H, F;
    if (G && B && J) {
        if (J instanceof YAHOO.util.DataSourceBase) this.dataSource = J;
        else return;
        if (this.key = 0, D = J.responseSchema, J._aDeprecatedSchema && (K = J._aDeprecatedSchema, YAHOO.lang.isArray(K) && (J.responseType === YAHOO.util.DataSourceBase.TYPE_JSON || J.responseType === YAHOO.util.DataSourceBase.TYPE_UNKNOWN ? (D.resultsList = K[0], this.key = K[1], D.fields = K.length < 3 ? null : K.slice(1)) : J.responseType === YAHOO.util.DataSourceBase.TYPE_XML ? (D.resultNode = K[0], this.key = K[1], D.fields = K.slice(1)) : J.responseType === YAHOO.util.DataSourceBase.TYPE_TEXT && (D.recordDelim = K[0], D.fieldDelim = K[1]), J.responseSchema = D)), YAHOO.util.Dom.inDocument(G)) YAHOO.lang.isString(G) ? (this._sName = "instance" + YAHOO.widget.AutoComplete._nIndex + " " + G, this._elTextbox = document.getElementById(G)) : (this._sName = G.id ? "instance" + YAHOO.widget.AutoComplete._nIndex + " " + G.id : "instance" + YAHOO.widget.AutoComplete._nIndex, this._elTextbox = G), YAHOO.util.Dom.addClass(this._elTextbox, "yui-ac-input");
        else return;
        if (YAHOO.util.Dom.inDocument(B)) this._elContainer = YAHOO.lang.isString(B) ? document.getElementById(B) : B, this._elContainer.style.display == "none", E = this._elContainer.parentNode, A = E.tagName.toLowerCase(), A == "div" && YAHOO.util.Dom.addClass(E, "yui-ac");
        else return;
        if (this.dataSource.dataType === YAHOO.util.DataSourceBase.TYPE_LOCAL && (this.applyLocalFilter = !0), C && C.constructor == Object)
            for (I in C) I && (this[I] = C[I]);
...
YAHOO.widget.AutoComplete.prototype.setHeader = function (B) {
    if (this._elHeader) {
        ...
        B ? (A.innerHTML = B, A.style.display = "block") : (A.innerHTML = "", A.style.display = "none")
    }
};
...
YAHOO.widget.AutoComplete.prototype.getSubsetMatches = function (E) {
    for (var D, C, A, B = E.length; B >= this.minQueryLength; B--)
        if (A = this.generateRequest(E.substr(0, B)), this.dataRequestEvent.fire(this, D, A), C = this.dataSource.getCachedResponse(A), C) return this.filterResults.apply(this.dataSource, [E, C, C, {
            scope: this
        }]);
    return null
};
YAHOO.widget.AutoComplete.prototype.preparseRawResponse = function (C, B) {
    ...
    return D != -1 && (B = B.substring(0, D)), B
};
YAHOO.widget.AutoComplete.prototype.filterResults = function (J, L, P, K) {
    var C, F, E, N, G;
    if (K && K.argument && K.argument.query && (J = K.argument.query), J && J !== "") {
        ...
        for (C = B.length - 1; C >= 0; C--) F = B[C], E = null, YAHOO.lang.isString(F) ? E = F : YAHOO.lang.isArray(F) ? E = F[0] : this.responseSchema.fields ? (N = this.responseSchema.fields[0].key || this.responseSchema.fields[0], E = F[N]) : this.key && (E = F[this.key]), YAHOO.lang.isString(E) && (G = I ? E.indexOf(decodeURIComponent(J)) : E.toLowerCase().indexOf(decodeURIComponent(J).toLowerCase()), (!A && G === 0 || A && G > -1) && M.unshift(F));
        P.results = M
    }
    return P
};
...
YAHOO.widget.AutoComplete.prototype.destroy = function () {
    ...
    D.innerHTML = "";
    for (C in this) YAHOO.lang.hasOwnProperty(this, C) && (this[C] = null)
};
...
YAHOO.widget.AutoComplete.prototype._initProps = function () {
    ...
    D = this.animSpeed;
    (this.animHoriz || this.animVert) && YAHOO.util.Anim && ((!YAHOO.lang.isNumber(D) || D < 0) && (this.animSpeed = .3), this._oAnim ? this._oAnim.duration = this.animSpeed : this._oAnim = new YAHOO.util.Anim(this._elContent, {}, this.animSpeed));
    this.forceSelection && A
};
YAHOO.widget.AutoComplete.prototype._initContainerHelperEls = function () {
    var A, B;
    this.useShadow && !this._elShadow && (A = document.createElement("div"), A.className = "yui-ac-shadow", A.style.width = 0, A.style.height = 0, this._elShadow = this._elContainer.appendChild(A));
    this.useIFrame && !this._elIFrame && (B = document.createElement("iframe"), B.src = this._iFrameSrc, B.frameBorder = 0, B.scrolling = "no", B.style.position = "absolute", B.style.width = 0, B.style.height = 0, B.tabIndex = -1, B.style.padding = 0, this._elIFrame = this._elContainer.appendChild(B))
};
YAHOO.widget.AutoComplete.prototype._initContainerEl = function () {
    var C, B, D, A;
    YAHOO.util.Dom.addClass(this._elContainer, "yui-ac-container");
    this._elContent || (C = document.createElement("div"), C.className = "yui-ac-content", C.style.display = "none", this._elContent = this._elContainer.appendChild(C), B = document.createElement("div"), B.className = "yui-ac-hd", B.style.display = "none", this._elHeader = this._elContent.appendChild(B), D = document.createElement("div"), D.className = "yui-ac-bd", this._elBody = this._elContent.appendChild(D), A = document.createElement("div"), A.className = "yui-ac-ft", A.style.display = "none", this._elFooter = this._elContent.appendChild(A))
};
YAHOO.widget.AutoComplete.prototype._initListEl = function () {
    for (var C = this.maxResultsDisplayed, A = this._elList || document.createElement("ul"), B, D; A.childNodes.length < C;) B = document.createElement("li"), B.style.display = "none", B._nItemIndex = A.childNodes.length, A.appendChild(B);
    this._elList || (D = this._elBody, YAHOO.util.Event.purgeElement(D, !0), D.innerHTML = "", this._elList = D.appendChild(A))
};
...
YAHOO.widget.AutoComplete.prototype._onInterval = function () {
    ...
    A != B && (this._sLastTextboxValue = A, this._sendQuery(A))
};
YAHOO.widget.AutoComplete.prototype._clearInterval = function () {
    this._queryInterval && (clearInterval(this._queryInterval), this._queryInterval = null)
};
...
YAHOO.widget.AutoComplete.prototype._sendQuery = function (D) {
    ...
    if (this.delimChar && (A = this._extractQuery(D), D = A.query, this._sPastSelections = A.previous), D && D.length < this.minQueryLength || !D && this.minQueryLength > 0) {
        this._nDelayID != -1 && clearTimeout(this._nDelayID);
        this._toggleContainer(!1);
        return
    }
    if (D = encodeURIComponent(D), this._nDelayID = -1, (this.dataSource.queryMatchSubset || this.queryMatchSubset) && (C = this.getSubsetMatches(D), C)) {
...
YAHOO.widget.AutoComplete.prototype._populateList = function (K, F, C) {
    ...
    if (this._nTypeAheadDelayID != -1 && clearTimeout(this._nTypeAheadDelayID), K = C && C.query ? C.query : K, H = this.doBeforeLoadData(K, F, C), H && !F.error) {
        if (this.dataReturnEvent.fire(this, K, F.results), this._bFocused || this._bFocused === null) {
            ...
            if (A > 0) {
                for ((!this._elList || this._elList.childNodes.length < A) && this._initListEl(), this._initContainerHelperEls(), I = this._elList.childNodes, Q = A - 1; Q >= 0; Q--) {
                    if (P = I[Q], E = R[Q], this.resultTypeList) {
                        if (B = [], B[0] = YAHOO.lang.isString(E) ? E : E[J] || E[this.key], L = this.dataSource.responseSchema.fields, YAHOO.lang.isArray(L) && L.length > 1)
                            for (N = 1, S = L.length; N < S; N++) B[B.length] = E[L[N].key || L[N]];
                        else YAHOO.lang.isArray(E) ? B = E : YAHOO.lang.isString(E) ? B = [E] : B[1] = E;
                        E = B
                    }
                ...
                if (A < I.length)
                    for (O = I.length - 1; O >= A; O--) G = I[O], G.style.display = "none";
                this._nDisplayedItems = A;
                this.containerPopulateEvent.fire(this, K, R);
                this.autoHighlight ? (D = this._elList.firstChild, this._toggleHighlight(D, "to"), this.itemArrowToEvent.fire(this, D), this._typeAhead(D, K)) : this._toggleHighlight(this._elCurListItem, "from");
                H = this.doBeforeExpandContainer(this._elTextbox, this._elContainer, K, R);
                this._toggleContainer(H)
            } else this._toggleContainer(!1);
            return
        }
    } else this.dataErrorEvent.fire(this, K)
};
...
YAHOO.widget.AutoComplete.prototype._textMatchesOption = function () {
    for (var C, D, A = null, B = 0; B < this._nDisplayedItems; B++)
        if (C = this._elList.childNodes[B], D = ("" + C._sResultMatch).toLowerCase(), D == this._sCurQuery.toLowerCase()) {
            A = C;
            break
        }
    return A
...
YAHOO.widget.AutoComplete.prototype._selectText = function (D, A, B) {
    if (D.setSelectionRange) D.setSelectionRange(A, B);
...
YAHOO.widget.AutoComplete.prototype._extractQuery = function (H) {
    for (var C = this.delimChar, F = -1, G, E, B = C.length - 1, D, A; B >= 0; B--) G = H.lastIndexOf(C[B]), G > F && (F = G);
    if (C[B] == " ")
...
YAHOO.widget.AutoComplete.prototype._toggleContainerHelpers = function (D) {
    var E = this._elContent.offsetWidth + "px",
        B = this._elContent.offsetHeight + "px",
        C, A;
    this.useIFrame && this._elIFrame && (C = this._elIFrame, D ? (C.style.width = E, C.style.height = B, C.style.padding = "") : (C.style.width = 0, C.style.height = 0, C.style.padding = 0));
    this.useShadow && this._elShadow && (A = this._elShadow, D ? (A.style.width = E, A.style.height = B) : (A.style.width = 0, A.style.height = 0))
};
YAHOO.widget.AutoComplete.prototype._toggleContainer = function (I) {
    var D = this._elContainer,
        A, G, H, J;
    if ((!this.alwaysShowContainer || !this._bContainerOpen) && (I || (this._toggleHighlight(this._elCurListItem, "from"), this._nDisplayedItems = 0, this._sCurQuery = null, this._elContent.style.display != "none")))
        if (A = this._oAnim, A && A.getEl() && (this.animHoriz || this.animVert)) {
            ...
            I && !this._bContainerOpen ? (this._elContent.style.width = B + "px", this._elContent.style.height = E + "px") : (this._elContent.style.width = F + "px", this._elContent.style.height = C + "px");
            D.removeChild(G);
            G = null;
            H = this;
            J = function () {
                A.onComplete.unsubscribeAll();
                I ? (H._toggleContainerHelpers(!0), H._bContainerOpen = I, H.containerExpandEvent.fire(H)) : (H._elContent.style.display = "none", H._bContainerOpen = I, H.containerCollapseEvent.fire(H))
            };
            this._toggleContainerHelpers(!1);
            this._elContent.style.display = "";
            A.onComplete.subscribe(J);
            A.animate()
        } else I ? (this._elContent.style.display = "", this._toggleContainerHelpers(!0), this._bContainerOpen = I, this.containerExpandEvent.fire(this)) : (this._toggleContainerHelpers(!1), this._elContent.style.display = "none", this._bContainerOpen = I, this.containerCollapseEvent.fire(this))
};
YAHOO.widget.AutoComplete.prototype._toggleHighlight = function (A, C) {
    if (A) {
        var B = this.highlightClassName;
        this._elCurListItem && (YAHOO.util.Dom.removeClass(this._elCurListItem, B), this._elCurListItem = null);
        C == "to" && B && (YAHOO.util.Dom.addClass(A, B), this._elCurListItem = A)
    }
};
...
YAHOO.widget.AutoComplete.prototype._updateValue = function (C) {
    ...
    if (!this.suppressInputUpdate) {
        var F = this._elTextbox,
            ...
            B = C._sResultMatch,
            D = "";
        E ? (D = this._sPastSelections, D += B + E, E != " " && (D += " ")) : D = B;
        F.value = D;
...
YAHOO.widget.AutoComplete.prototype._moveSelection = function (G) {
    ...
    if (this._bContainerOpen) {
        if (H = this._elCurListItem, D = -1, H && (D = H._nItemIndex), E = G == 40 ? D + 1 : D - 1, E < -2 || E >= this._nDisplayedItems) return;
        if (H && (this._toggleHighlight(H, "from"), this.itemArrowFromEvent.fire(this, H)), E == -1) {
...
YAHOO.widget.AutoComplete.prototype._onTextboxKeyDown = function (A, B) {
    ...
    switch (C) {
    case 9:
        (!YAHOO.env.ua.opera && navigator.userAgent.toLowerCase().indexOf("mac") == -1 || YAHOO.env.ua.webkit > 420) && (B._elCurListItem ? (B.delimChar && B._nKeyCode != C && B._bContainerOpen && YAHOO.util.Event.stopEvent(A), B._selectItem(B._elCurListItem)) : B._toggleContainer(!1));
        break;
    case 13:
        (!YAHOO.env.ua.opera && navigator.userAgent.toLowerCase().indexOf("mac") == -1 || YAHOO.env.ua.webkit > 420) && (B._elCurListItem ? (B._nKeyCode != C && B._bContainerOpen && YAHOO.util.Event.stopEvent(A), B._selectItem(B._elCurListItem)) : B._toggleContainer(!1));
        break;
    case 27:
    ...
    case 39:
        ...
        break;
    case 38:
        B._bContainerOpen && (YAHOO.util.Event.stopEvent(A), B._moveSelection(C));
        break;
    case 40:
        B._bContainerOpen && (YAHOO.util.Event.stopEvent(A), B._moveSelection(C));
        break;
...
YAHOO.widget.AutoComplete.prototype._onTextboxKeyPress = function (A, B) {
    var C = A.keyCode;
    if (YAHOO.env.ua.opera || navigator.userAgent.toLowerCase().indexOf("mac") != -1 && YAHOO.env.ua.webkit < 420) switch (C) {
    case 9:
        B._bContainerOpen && (B.delimChar && YAHOO.util.Event.stopEvent(A), B._elCurListItem ? B._selectItem(B._elCurListItem) : B._toggleContainer(!1));
        break;
    case 13:
        B._bContainerOpen && (YAHOO.util.Event.stopEvent(A), B._elCurListItem ? B._selectItem(B._elCurListItem) : B._toggleContainer(!1))
    } else C == 229 && B._enableIntervalDetection()
};
YAHOO.widget.AutoComplete.prototype._onTextboxKeyUp = function (A, C) {
    var B = this.value,
        D;
    (C._initProps(), D = A.keyCode, C._isIgnoreKey(D)) || (C._nDelayID != -1 && clearTimeout(C._nDelayID), C._nDelayID = setTimeout(function () {
        C._sendQuery(B)
    }, C.queryDelay * 1e3))
};
YAHOO.widget.AutoComplete.prototype._onTextboxFocus = function (A, B) {
    B._bFocused || (B._elTextbox.setAttribute("autocomplete", "off"), B._bFocused = !0, B._sInitInputValue = B._elTextbox.value, B.textboxFocusEvent.fire(B))
};
YAHOO.widget.AutoComplete.prototype._onTextboxBlur = function (A, C) {
    if (C._bOverContainer && C._nKeyCode != 9) C._focus();
    else {
        if (!C._bItemSelected) {
...
YAHOO.widget.AutoComplete.prototype.getListItems = function () {
    for (var C = [], B = this._elList.childNodes, A = B.length - 1; A >= 0; A--) C[A] = B[A];
    return C
};
YAHOO.widget.AutoComplete._cloneObject = function (D) {
    var F, E, C, B, A;
    if (!YAHOO.lang.isValue(D)) return D;
    if (F = {}, YAHOO.lang.isFunction(D)) F = D;
    else if (YAHOO.lang.isArray(D)) {
        for (E = [], C = 0, B = D.length; C < B; C++) E[C] = YAHOO.widget.AutoComplete._cloneObject(D[C]);
        F = E
    } else if (YAHOO.lang.isObject(D))
        for (A in D) YAHOO.lang.hasOwnProperty(D, A) && (F[A] = YAHOO.lang.isValue(D[A]) && YAHOO.lang.isObject(D[A]) || YAHOO.lang.isArray(D[A]) ? YAHOO.widget.AutoComplete._cloneObject(D[A]) : D[A]);
    else F = D;
    return F
};
