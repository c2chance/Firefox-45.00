...
s.apl=new Function("l","v","d","u",""+"var s=this,m=0;if(!l)l='';if(u){var i,n,a=s.split(l,d);for(i=0;i<a."+"length;i++){n=a[i];m=m||(u==1?(n==v):(n.toLowerCase()==v.toLowerCas"+"e()));}}if(!m)l=l?l+d+v:v;return l");function stripParameters(){var loc=window.location.href.split("?")[0];return loc.split("?")[0];}
var fs_url=document.location.href;if(s.c_r('fs_dnaQ')){var fs_cV=s.c_r('fs_dnaQ');var fs_cc=fs_cV.split(',');var fs_ccL=fs_cc.length;}
...
if(typeof(trackingPageGuid)!='undefined'){if(trackingPageGuid=='8ee9f512-ecc6-4722-94de-7bf45616f749'){b=loginUdo;}
if(trackingPageGuid=='c5e11535-b263-48ca-90a5-e7d647cd0be4'||trackingPageGuid=='86fad009-cd35-4f70-b37b-f2eb360f47e5'){s.eVar36='DNA Kit';s.products='AncestryDNA Kit;DNA Kit;1;'+utag.data['order_total'];s.events='purchase,event76';s.prop39='DNA Kit';}
if(trackingPageGuid=='2e37b21b-2e60-457c-8852-4d4eb9c7f112'){if(utag.data['flow_type']=='hard offer'){s.prop39='Hard Offer';}
if(utag.data['flow_type']=='free trial'){s.prop39='Free Trial';}
if(utag.data['flow_type']=='upgrade'){s.prop39='Upgrade';}}}
var pName='none : none : none : needs valid page name';if(b['page_name']){pName=b['page_name'];}
if(pName.indexOf(' : ')==-1){pName=pName.replace(/:/g," : ");}
if(loc.indexOf('wiz2.ancestry')!=-1){if(b['page_name']===undefined){return false;}
if(b['page_name'].indexOf('wiz2')!=-1||loc.indexOf('wiz2.ancestry')!=-1||b['page_name'].indexOf('merlin : forms : reg-popup')!=-1){if(pName==''){pName=b['page_name'];}
...
if((utag.data['dom.url'].indexOf('home.ancestry'.toLowerCase())>-1)&&(utag.data['dom.pathname']=='/')){pName='ancestry us : home page : logged in : original';}
if(loc.indexOf('wiz2.ancestry.com/Records/SEO~Records')!=-1){pName='ancestry '+domainExt+' : wiz2 : seo : '+locP[5];}
if(loc.indexOf('/ng/signup')!=-1&&window.location.hash=='#/complete/'){var dnaTotal=document.getElementById('total').innerHTML;var dnaQty=document.getElementById('quantity').innerHTML;dnaTotal=dnaTotal.replace("$","");s.events='event76,purchase';s.products='DNA Kit;DNA Kit;'+dnaQty+';'+dnaTotal;s.prop39='DNA Kit';if(b['cp.JSESSIONID']=="dummy"){s.prop39='Test Order : '+s.prop39;}}
if(pNameA=pName.split(' : ')){s.hier1=pName.replace(/ : /g,':');s.channel=pNameA[1];s.prop42=pNameA[0]+' : '+pNameA[1];s.prop43=pNameA[0]+' : '+pNameA[1]+' : '+pNameA[2];s.eVar42="D=c42";s.eVar43="D=c43";}
if(b['order_id']){if(b['flow_type']=="gift sub - upgrade"){s.events='event34';}
if(b['flow_type']=="gift sub - downgrade"){s.events='event35';}
if(b['flow_type']=="free trial"){s.events='event32';}
if(b['flow_type']=="upgrade"){s.events='event34';}
if(b['flow_type']=="hard offer"){s.events='event33';}
if(b['flow_type']=="gift sub"){s.events='event36';}}}
...
pName=pName.toLowerCase();b['page_name']=pName;if(loc.indexOf('blogs.ancestry')!=-1){b['page_name']=loc;s.prop42="blogs";s.prop43="blogs";s.eVar42="blogs";s.eVar43="blogs";}
s.prop42=s.prop42.toLowerCase();s.pageName=b['page_name'];s.pageName=window.s.pageName;s.server=window.document.location.hostname;s.currencyCode='USD';s.campaign=s.getQueryParam('o_xid');s.trackInlineStats=true;s.charSet="UTF-8";s.cookieDomainPeriods=document.location.hostname.length-document.location.hostname.replace(/\./g,'').length;s.pageURL=s.manageQueryParam('s_kwcid',1,1);if(s.channel==''){s.channel='unknown';}
if((s.pageURL.match(/deny.aspx/g))||(s.pageURL.match(/offers\/join/g))){s.events='event3';}
...
s.prop5='';s.eVar4=s.getQueryParam('o_iid');if(b['order_id']){s.eVar8=b['order_id'];}
...
s.prop20=s.getQueryParam('o_ufc');s.prop21=s.getQueryParam('o_cvc');if(s.getQueryParam('cat')){s.prop22=s.getQueryParam('cat')+':n:category';s.eVar22='D=c22';}
...
if(b['cp.BAIT']){var sc_bait=b['cp.BAIT'];if(sc_bait.indexOf('CSub=0')>=1&&sc_bait.indexOf('ESub=1')>=1){s.prop24="win back";}
else{s.prop24="not win back";}
if(sc_bait.indexOf('NewDna=1')!=-1){s.prop35='DNA User';s.eVar35='D=c35';}
else if(sc_bait.indexOf('NewDna=1')==-1){s.prop35='Non DNA User';s.eVar35='D=c35';}}
...
if(typeof(TaggerData)!='undefined'){if(TaggerData.Id=='e499e543-3cef-4255-a3e1-bf13d2e71283'){if(b['flow_type']=='hard offer'){s.prop39='Hard Offer';}
if(b['flow_type']=='free trial'){s.prop39='Free Trial';}
if(b['flow_type']=='upgrade'){s.prop39='Upgrade';}}
if(TaggerData.Id=='4aca975c-ff6c-4f4a-8794-1397f83562fc'){if(b['flow_type']=='gift sub'){s.prop39='Gift Sub';}}}
...
s.prop44=dateFormat(now);if(s.prop44!=''){s.eVar44='D=c44';}
...
if(s.getQueryParam('utm_source')){cm_source='unknown';cm_site='unknown';cm_source=s.getQueryParam('utm_source').toLowerCase();if(s.getQueryParam('utm_site')&&(cm_source=="taboola")){cm_site=s.getQueryParam('utm_site').toLowerCase();}else if(s.getQueryParam('utm_term')&&(cm_source=="outbrain")){cm_site=s.getQueryParam('utm_term').toLowerCase();}
if(cm_source==''){cm_source='unknown';}if(cm_site==''){cm_source='unknown';}
s.eVar66=cm_source+" : "+cm_site;}else{s.eVar66='';}
s.eVar74=s.getQueryParam('gclid');if(document.referrer.indexOf('ancestry.com/wiki')!=-1){s.prop70="wiki referred";}
if(s.getQueryParam('email_segment')){s.eVar70="email segment: "+s.getQueryParam('email_segment');}
if(b.action){if(b.action=='none'){return false;}
s.prop70=b.action;sc_npv('prop70',s.prop70,0);return false;}
if(b['sc_event']){if(b['sc_event'].indexOf('event1')!=-1){s.events=s.apl(s.events,'event1',',',0);}
...
if(typeof(bagString)!='undefined'){utag_data['bagString']=bagString;}
...
if(s.visitorMigrationKey||s_visitorMigrationKey){return false;}}];u.send=function(a,b,c,d,e,f,g,h,ev,evt){if(u.ev[a]||typeof u.ev.all!="undefined"){utag.DB("send:24");for(c=0;c<u.extend.length;c++){try{d=u.extend[c](a,b);if(d==false)return}catch(e){}};if(a=="link"){u.ltflag=true;if(typeof b.linkTrackVars=="undefined"){u.ltv=[]}
