var $genTreesModal = {
    ...
    create: function (id, url, htmlSrc, callback) {
        if (url != null && url == "0" && htmlSrc != null && htmlSrc.length > 0) return $genTreesModal.getView(id, htmlSrc), $("#modalHeader").hide(), callback != null && callback(), $.modal.center(), !0;
        if (htmlSrc != null && htmlSrc.length > 0) {
            var htmlSrcGet = $("#" + htmlSrc);
            htmlSrcGet && ($genTreesModal.getView(id, htmlSrcGet.html()), $("#modalHeader").hide(), callback != null && callback(), $.modal.center())
        } else $.ajax({
            url: url,
            cache: !1,
            timeout: 2e4,
            ...
            success: function (data) {
                ...
                modalTitle.length > 0 && $genTreesModal.TitleName.length > 0 && (titleVal = modalTitle.data($genTreesModal.TitleName), $("#modalHeader").text(titleVal), $("#modalHeader").show(), $.modal.title(titleVal));
    ...
    getView: function (id, html) {
        if (html == null || html.responseText == "fail") $genTreesModal.throwException(id, html);
