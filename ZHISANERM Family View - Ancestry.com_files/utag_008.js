//tealium universal tag - utag.24 ut4.0.201602261854, Copyright 2016 Tealium.com Inc. All Rights Reserved.
...
s.linkDownloadFileTypes = "zip,exe,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx";
s.linkInternalFilters = ".ancestry.,ancestrydev,ancestrystage,javascript";
s.linkLeaveQueryString = false;
s.linkTrackVars = "None";
s.linkTrackEvents = "None";
s.usePlugins = false;
s.currencyCode = "USD";
s.visitorNamespace = "ancestry";
s.trackingServer = "metrics.ancestry.com";
s.trackingServerSecure = "smetrics.ancestry.com";
s.dc = 122;
var s_code = '',
    s_objectID;
...
try {
    (function (id, loader, u) {
        ...
        u.map = {
            "page_name": "eVar41,pageName",
            "beanstalk_invite": "prop56,eVar56",
            "beanstalk_node": "eVar55,prop55",
            "campaign": "campaign",
            "content_usage": "eVar18",
            "content_view": "eVar11",
            "content_view_type": "prop15",
            "customer_segment": "prop12",
            "db_view": "prop1",
            "ext_cmp": "prop53",
            "flow_type": "prop36,eVar60",
            "hint_accepted": "eVar26",
            "hint_filters": "prop34",
            "img_view_src": "prop47",
            "int_cmp": "prop54",
            "last_db_denied": "eVar1",
            "last_flow_type": "eVar47",
            "mrkt_region": "prop38,eVar38",
            "new_img_viewr_action": "prop40,eVar40",
            "offer_id": "eVar54",
            "offer_pg_view": "eVar61",
            "org_srch_query": "eVar52",
            "place_gpid": "prop35",
            "product_type": "prop10,eVar15",
            "reg_type": "eVar48",
            "related_rcds_src": "eVar36",
            "related_rcds_tgt": "eVar37",
            "searc_dbid": "prop22,eVar22",
            "search_type": "prop46",
            "server": "server",
            "site_cobrand": "eVar33,prop33",
            "srch_center": "eVar51",
            "subscription_type": "eVar15",
            "tmp_eVar": "eVar70",
            "tmp_prop": "prop70",
            "tree_hints": "eVar5",
            "ucdmid": "prop11,prop67",
            "webservice_id": "prop51",
            "order_id": "eVar8",
            "cp.ANCUUID": "prop5",
            "dom.domain": "prop68",
            "bait_eTrial": "eVar73"
        ...
        u.extend = [function (a, b) {
            ...
            function currentTime() {
                var sc_date_d = new Date();
                var sc_date_offset = "-7";
                var sc_date_utc = sc_date_d.getTime() + (sc_date_d.getTimezoneOffset() * 60000);
                var sc_date = new Date(sc_date_utc + (3600000 * sc_date_offset));
                var sc_day = ('0' + sc_date.getDate()).slice(-2);
                var sc_month = ('0' + (sc_date.getMonth() + 1)).slice(-2);
                var sc_year = sc_date.getFullYear();
                var sc_hour = ('0' + sc_date.getHours()).slice(-2);
                var sc_minute = ('0' + sc_date.getMinutes()).slice(-2);
                var sc_seconds = ('0' + sc_date.getSeconds()).slice(-2);
                var sc_timezone = sc_date.getTimezoneOffset();
                var fulldate = sc_month + '/' + sc_day + '/' + sc_year + ' ' + sc_hour + ':' + sc_minute + ':' + sc_seconds;
                return (fulldate);
            }
            ...
            function keepVars(kVar) {
                for (var i = 0; i < 100; i++) {
                    if (!kVar.match(new RegExp('\\b' + 'c' + i + '\\b', 'g'))) {
                        s['prop' + i] = '';
                    }
                    if (!kVar.match(new RegExp('\\b' + 'v' + i + '\\b', 'g'))) {
                        s['eVar' + i] = '';
                    }
                    if (i <= 5) {
                        if (!kVar.match(new RegExp('\\b' + 'h' + i + '\\b', 'g'))) {
                            s['hier' + i] = '';
                        }
                    }
                }
                ...
                for (var i = 0; i < svarArr.length; i++) {
                    if (!kVar.match(new RegExp('\\b' + svarArr[i] + '\\b', 'g'))) {
                        s[svarArr[i]] = '';
                    }
                }
            }

            function sc_npv(v, va, d) {
                s.linkTrackVars = "";
                s.linkTrackEvents = "";
                var tD = new Date();
                var cD = new Date(d);
                if (tD < cD || d == 0) {
                    ...
                    for (var i = 0; i < vAr.length; i++) {
                        ...
                        if (s[vAr[i]].match(/event/g)) {
                            s.linkTrackEvents += vARX[0] + ",";
                            s.linkTrackVars += 'events,';
                        } else {
                            s.linkTrackVars += vAr[i] + ",";
                        }
                    }
                    if (s.linkTrackVars.substring(s.linkTrackVars.length - 1, s.linkTrackVars.length) == ',') {
                        s.linkTrackVars = s.linkTrackVars.substring(0, s.linkTrackVars.length - 1);
                    }
                    if (s.linkTrackEvents.substring(s.linkTrackEvents.length - 1, s.linkTrackEvents.length) == ',') {
                        s.linkTrackEvents = s.linkTrackEvents.substring(0, s.linkTrackEvents.length - 1);
                    }
            ...
            var fs_url = document.location.href;
            if (s.c_r('fs_dnaQ')) {
                var fs_cV = s.c_r('fs_dnaQ');
                var fs_cc = fs_cV.split(',');
                var fs_ccL = fs_cc.length;
            } else {
                s.c_w('fs_dnaQ', '0', 0);
                var fs_cV = '0';
            }
            if (fs_url.indexOf('dna.ancestry.com/#/tests') != -1 || fs_url.indexOf('dna.ancestry.com/#/matches') != -1 || fs_url.indexOf('dna.ancestry.com/#/ethnicity') != -1) {
                if (fs_url.indexOf('dna.ancestry.com/#/tests') != -1) {
                    if (fs_cV.indexOf('dna.ancestry.com/#/tests') == -1) {
                        fs_cV = fs_cV + ',' + fs_url;
                    }
                }
                if (fs_url.indexOf('dna.ancestry.com/#/matches') != -1) {
                    if (fs_cV.indexOf('dna.ancestry.com/#/matches') == -1) {
                        fs_cV = fs_cV + ',' + fs_url;
                    }
                }
                if (fs_url.indexOf('dna.ancestry.com/#/ethnicity') != -1) {
                    if (fs_cV.indexOf('dna.ancestry.com/#/ethnicity') == -1) {
                        fs_cV = fs_cV + ',' + fs_url;
                    }
                }
                s.c_w('fs_dnaQ', fs_cV, 0);
            }
            if (fs_ccL >= 3) {
                s.c_w('fs_dna', 'true', 0);
            }
            ...
            if (typeof (utag.data['trackingPageGuid']) != 'undefined') {
                if (utag.data['trackingPageGuid'] == '867663FA-EDD9-49F6-84F8-969F8CEF7FBA') {
                    s.eVar36 = 'DNA Kit';
                    s.products = 'AncestryDNA Kit;DNA Kit;1;' + utag.data['order_total'];
                    s.events = 'purchase,event76';
                    s.prop39 = 'DNA Kit';
                }
            }
            s.tnt = s.trackTNT();
            ...
            if (domain[1].indexOf('ancestrydev') != -1) {
                s.trackingServer = 'metrics.ancestrydev.com';
                s.trackingServerSecure = 'smetrics.ancestry.com';
                s.sa('devancestry');
                s_account = 'devancestry';
            } else if (domain[1].indexOf('ancestrystage') != -1) {
                s.trackingServer = 'metrics.ancestrystage.com';
                s.trackingServerSecure = 'smetrics.ancestrystage.com';
                s.sa('ancestrystage');
                s_account = 'ancestrystage';
            } else if (locP[3] == 'wiki') {
                s.trackingServer = 'metrics.ancestry.com';
                s.trackingServerSecure = 'smetrics.ancestry.com';
                s.sa('ancestrywiki');
                s_account = 'ancestrywiki';
            } else if (domain[1].indexOf('ancestrylibrary') != -1) {
                s.sa('ancestrylibrary,myfamilyancestry');
                s_account = 'ancestrylibrary,myfamilyancestry';
            } else if (domain[1].indexOf('ancestryinstitution') != -1) {
                s.sa('ancestrylibrary,myfamilyancestry');
                s_account = 'ancestrylibrary,myfamilyancestry';
            } else if (domain[1].indexOf('ancestryclassroom') != -1) {
                s.sa('ancestrylibrary,myfamilyancestry');
                s_account = 'ancestrylibrary,myfamilyancestry';
            } else if (domainExt == 'au') {
                s.trackingServer = 'metrics.ancestry.com.au';
                s.trackingServerSecure = 'smetrics.ancestry.com.au';
                s.sa('ancestrycomau');
                s_account = 'ancestrycomau';
            } else if (domainExt == 'ca') {
                s.trackingServer = 'metrics.ancestry.ca';
                s.trackingServerSecure = 'smetrics.ancestry.ca';
                s.sa('ancestryca');
                s_account = 'ancestryca';
            } else if (domainExt == 'fr') {
                s.trackingServer = 'metrics.ancestry.fr';
                s.trackingServerSecure = 'smetrics.ancestry.fr';
                s.sa('ancestryfr');
                s_account = 'ancestryfr';
            } else if (domainExt == 'de') {
                s.trackingServer = 'metrics.ancestry.de';
                s.trackingServerSecure = 'smetrics.ancestry.de';
                s.sa('ancestryde');
                s_account = 'ancestryde';
            } else if (domainExt == 'ie') {
                s.trackingServer = 'metrics.ancestry.ie';
                s.trackingServerSecure = 'smetrics.ancestry.ie';
                s.sa('ancestryie');
                s_account = 'ancestryie';
            } else if (domainExt == 'it') {
                s.trackingServer = 'metrics.ancestry.it';
                s.trackingServerSecure = 'smetrics.ancestry.it';
                s.sa('ancestryit');
                s_account = 'ancestryit';
            } else if (domainExt == 'com') {
                s.trackingServer = 'metrics.ancestry.com';
                s.trackingServerSecure = 'smetrics.ancestry.com';
                s.sa('myfamilyancestry');
                s_account = 'myfamilyancestry';
                domainExt = 'us';
            } else if (domainExt == 'mx') {
                s.trackingServer = 'metrics.ancestry.mx';
                s.trackingServerSecure = 'smetrics.ancestry.mx';
                s.sa('ancestrymexico');
                s_account = 'ancestrymexico';
            } else if (domainExt == 'se') {
                s.trackingServer = 'metrics.ancestry.se';
                s.trackingServerSecure = 'smetrics.ancestry.se';
                s.sa('ancestryse');
                s_account = 'ancestryse';
            } else if (domainExt == 'uk') {
                s.trackingServer = 'metrics.ancestry.co.uk';
                s.trackingServerSecure = 'smetrics.ancestry.co.uk';
                s.sa('ancestryuki');
                s_account = 'ancestryuki';
            } else if (domain[0].indexOf('health') != -1) {
                s.sa('ancestryhealth');
                s_account = 'ancestryhealth';
            } else if (domain[1].indexOf('familyhistoryhealth') != -1) {
                s.sa('ancestryhealth');
                s_account = 'ancestryhealth';
            } else if (domain[0].indexOf('genealogy') != -1 || domain[1].indexOf('genealogy') != -1) {
                s.sa('gcom,myfamilyancestry');
                s_account = 'gcom,myfamilyancestry';
            } else if (domain[0].indexOf('blogs') != -1) {
                s.sa('myfamilyancblog,myfamilyancestry');
                s_account = 'myfamilyancblog,myfamilyancestry';
            } else {
                s.sa('myfamilyancestry');
            }
            if (typeof (trackingPageGuid) != 'undefined') {
                if (trackingPageGuid == '8ee9f512-ecc6-4722-94de-7bf45616f749') {
                    b = loginUdo;
                }
                if (trackingPageGuid == 'c5e11535-b263-48ca-90a5-e7d647cd0be4' || trackingPageGuid == '86fad009-cd35-4f70-b37b-f2eb360f47e5') {
                    s.eVar36 = 'DNA Kit';
                    s.products = 'AncestryDNA Kit;DNA Kit;1;' + utag.data['order_total'];
                    s.events = 'purchase,event76';
                    s.prop39 = 'DNA Kit';
                }
                if (trackingPageGuid == '2e37b21b-2e60-457c-8852-4d4eb9c7f112') {
            ...
            if (loc.indexOf('wiz2.ancestry') != -1) {
                if (b['page_name'] === undefined) {
                    return false;
                }
                if (b['page_name'].indexOf('wiz2') != -1 || loc.indexOf('wiz2.ancestry') != -1 || b['page_name'].indexOf('merlin : forms : reg-popup') != -1) {
                    if (pName == '') {
                        pName = b['page_name'];
                    }
                    if (pName === undefined) {
                    ...
                    if (pNameA = pName.split(' : ')) {
                        s.hier1 = pName.replace(/ : /g, ':');
                        s.channel = pNameA[1];
                        s.prop42 = pNameA[0] + ' : ' + pNameA[1];
                        s.prop43 = pNameA[0] + ' : ' + pNameA[1] + ' : ' + pNameA[2];
                        s.eVar42 = "D=c42";
                        s.eVar43 = "D=c43";
                    }
                }
            ...
            if (typeof (utag.data) != 'undefined') {
                ...
                if ((utag.data['dom.url'].indexOf('home.ancestry'.toLowerCase()) > -1) && (utag.data['dom.pathname'] == '/')) {
                    pName = 'ancestry us : home page : logged in : original';
                }
                if (loc.indexOf('wiz2.ancestry.com/Records/SEO~Records') != -1) {
                    pName = 'ancestry ' + domainExt + ' : wiz2 : seo : ' + locP[5];
                }
                if (loc.indexOf('/ng/signup') != -1 && window.location.hash == '#/complete/') {
                    var dnaTotal = document.getElementById('total').innerHTML;
                    var dnaQty = document.getElementById('quantity').innerHTML;
                    dnaTotal = dnaTotal.replace("$", "");
                    s.events = 'event76,purchase';
                    s.products = 'DNA Kit;DNA Kit;' + dnaQty + ';' + dnaTotal;
                    s.prop39 = 'DNA Kit';
                    if (b['cp.JSESSIONID'] == "dummy") {
                        s.prop39 = 'Test Order : ' + s.prop39;
                    }
                }
                if (pNameA = pName.split(' : ')) {
                    s.hier1 = pName.replace(/ : /g, ':');
                ...
                if (b['order_id']) {
                    if (b['flow_type'] == "gift sub - upgrade") {
                        s.events = 'event34';
                    }
                    if (b['flow_type'] == "gift sub - downgrade") {
                        s.events = 'event35';
                    }
                    if (b['flow_type'] == "free trial") {
                        s.events = 'event32';
                    }
                    if (b['flow_type'] == "upgrade") {
                        s.events = 'event34';
                    }
                    if (b['flow_type'] == "hard offer") {
                        s.events = 'event33';
                    }
                    if (b['flow_type'] == "gift sub") {
                        s.events = 'event36';
                    }
                }
            }
            if (typeof pName == "undefined" || pName == '') {
                pName = 'none : none : none : needs valid page name';
            }
            ...
            b['page_name'] = pName;
            if (loc.indexOf('blogs.ancestry') != -1) {
                b['page_name'] = loc;
                s.prop42 = "blogs";
                s.prop43 = "blogs";
                s.eVar42 = "blogs";
            ...
            s.prop42 = s.prop42.toLowerCase();
            s.pageName = b['page_name'];
            s.pageName = window.s.pageName;
            s.server = window.document.location.hostname;
            s.currencyCode = 'USD';
            s.campaign = s.getQueryParam('o_xid');
            s.trackInlineStats = true;
            s.charSet = "UTF-8";
            s.cookieDomainPeriods = document.location.hostname.length - document.location.hostname.replace(/\./g, '').length;
            s.pageURL = s.manageQueryParam('s_kwcid', 1, 1);
            if (s.channel == '') {
                s.channel = 'unknown';
            }
            if ((s.pageURL.match(/deny.aspx/g)) || (s.pageURL.match(/offers\/join/g))) {
                s.events = 'event3';
            }
            if (s.getQueryParam('dbid')) {
                s.eVar1 = s.getQueryParam('dbid');
            }
            if (b['db_view']) {
                s.prop1 = b['db_view'];
            }
            if (b['search_dbid']) {
                s.eVar1 = b['search_dbid'];
            } else {
            ...
            if (b['product_type']) {
                s.prop10 = b['product_type'];
            }
            if (s.c_r('LAU')) {
                s.prop11 = s.c_r('LAU');
            } else {
                s.prop11 = '00000000-0000-0000-0000-000000000000';
            }
            if (b['content_view']) {
                s.eVar11 = b['content_view'];
            }
            if (b['search_resultsQuality']) {
                s.prop15 = b['search_resultsQuality'];
            }
            if (b['search_category']) {
                s.prop16 = b['search_category'];
            }
            if (b['search_subcategory']) {
                s.prop17 = b['search_subcategory'];
            }
            if (b['search_dbName']) {
                s.prop18 = b['search_dbName'];
            }
            if (s.prop10 != '') {
                s.eVar15 = 'D=c10';
            }
            s.prop20 = s.getQueryParam('o_ufc');
            s.prop21 = s.getQueryParam('o_cvc');
            if (s.getQueryParam('cat')) {
                s.prop22 = s.getQueryParam('cat') + ':n:category';
                s.eVar22 = 'D=c22';
            }
            if (s.c_r('OMNITURE')) {
                s.prop23 = s.c_r('OMNITURE').substring(s.c_r('OMNITURE').indexOf('=') + 1);
            } else {
                s.prop23 = "NRVisitor";
            }
            if (s.prop23 != '') {
            ...
            if (b['cp.BAIT']) {
                var sc_bait = b['cp.BAIT'];
                if (sc_bait.indexOf('CSub=0') >= 1 && sc_bait.indexOf('ESub=1') >= 1) {
                    s.prop24 = "win back";
                } else {
                    s.prop24 = "not win back";
                }
                if (sc_bait.indexOf('NewDna=1') != -1) {
                    s.prop35 = 'DNA User';
                    s.eVar35 = 'D=c35';
                } else if (sc_bait.indexOf('NewDna=1') == -1) {
                    s.prop35 = 'Non DNA User';
                    s.eVar35 = 'D=c35';
                }
            }
            if (b['search_type'] == 'New Search') {
                s.events = 'event31';
            }
            if (b['hint_accepted']) {
                s.eVar26 = b['hint_accepted'];
            }
            if (b['flow_type']) {
                s.prop36 = b['flow_type'];
            }
            if (typeof (TaggerData) != 'undefined') {
                if (TaggerData.Id == 'e499e543-3cef-4255-a3e1-bf13d2e71283') {
                    if (b['flow_type'] == 'hard offer') {
                        s.prop39 = 'Hard Offer';
                    }
                    if (b['flow_type'] == 'free trial') {
                ...
                if (TaggerData.Id == '4aca975c-ff6c-4f4a-8794-1397f83562fc') {
                    if (b['flow_type'] == 'gift sub') {
                        s.prop39 = 'Gift Sub';
                    }
                }
            }
            if (b['test_order'] == true) {
                s.prop39 = 'Test Order : ' + s.prop39;
            }
            if (s.prop39 != '') {
                s.eVar39 = 'D=c39';
            }
            if (b['new_img_viewr_action']) {
                s.prop40 = b['new_img_viewr_action'];
                s.eVar40 = 'D=c40';
            }
            s.prop41 = s.getPreviousValue(b['page_name'], 'gpv_pn');
            if (s.pagName != '') {
                s.eVar41 = 'D=pageName';
            }
            s.prop44 = dateFormat(now);
            if (s.prop44 != '') {
                s.eVar44 = 'D=c44';
            }
            s.prop45 = loc;
            ...
            if (typeof (utag.data['dbCategory']) == 'undefined') {
                utag.data['dbCategory'] = s.getQueryParam('cat');
            }
            s.prop65 = utag.data['dbCategory'];
            if (s.prop11 != '') {
                s.eVar65 = 'D=c11';
            }
            s.prop66 = s.getQueryParam('archvkw');
            ...
            if (s.getQueryParam('utm_source')) {
                cm_source = 'unknown';
                cm_site = 'unknown';
                cm_source = s.getQueryParam('utm_source').toLowerCase();
                if (s.getQueryParam('utm_site') && (cm_source == "taboola")) {
                    cm_site = s.getQueryParam('utm_site').toLowerCase();
                } else if (s.getQueryParam('utm_term') && (cm_source == "outbrain")) {
                    cm_site = s.getQueryParam('utm_term').toLowerCase();
                }
                if (cm_source == '') {
                    cm_source = 'unknown';
                }
                ...
                s.eVar66 = cm_source + " : " + cm_site;
            } else {
                s.eVar66 = '';
            }
            s.eVar74 = s.getQueryParam('gclid');
            if (document.referrer.indexOf('ancestry.com/wiki') != -1) {
                s.prop70 = "wiki referred";
            }
            if (s.getQueryParam('email_segment')) {
                s.eVar70 = "email segment: " + s.getQueryParam('email_segment');
            }
            if (b.action) {
                if (b.action == 'none') {
                    return false;
                ...
                s.prop70 = b.action;
                sc_npv('prop70', s.prop70, 0);
                return false;
            }
            if (b['sc_event']) {
                if (b['sc_event'].indexOf('event1') != -1) {
                    s.events = s.apl(s.events, 'event1', ',', 0);
                }
                if (b['sc_event'].indexOf('event21') != -1) {
                    s.events = s.apl(s.events, 'event21', ',', 0);
                }
            }
            if (typeof (b['dbSection']) != 'undefined') {
                if (b['dbSection'] == 'military') {
                    s.prop70 = 'search category : military';
                }
            }
            if (typeof (b['search_category']) != 'undefined') {
                if (b['search_category'] == 'Military') {
                    s.prop70 = 'search category : military';
                }
            }
            if (typeof (bagString) != 'undefined') {
                utag_data['bagString'] = bagString;
            }
            if (s.getQueryParam('subid')) {
                s.c_w('an_subid', s.getQueryParam('subid'), 0);
            }
            if (s.getQueryParam('sub_ad')) {
                s.c_w('an_sub_ad', s.getQueryParam('sub_ad'), 0);
            }
            if (((utag.data['dom.url'].indexOf('trees.ancestry') != -1) && (utag.data['dom.pathname'].indexOf('/tree/') != -1)) || ((utag.data['dom.domain'].toLowerCase() == 'wiz2.ancestry.com') && (utag.data['dom.hash'].toLowerCase() == '/searchresults') && (typeof (utag.data['cp.LAU']) != 'undefined') && (utag.data['cp.LAU'] != '00000000-0000-0000-0000-000000000000')) || ((utag.data['bait_hasTree'] >= 1))) {
                var e = new Date();
                e.setDate(e.getDate() + 30);
                s.c_w('an_hasTree', '1', e);
            }
            if (utag.data['dom.query_string'].indexOf('o_xid=69630') >= 1) {
                var anc_d = new Date();
                anc_d.setTime(anc_d.getTime() + (30 * 24 * 60 * 60 * 1000));
                var anc_expires = "expires=" + anc_d.toUTCString();
                document.cookie = "anc_pch=true; " + anc_expires + "; domain=.ancestry.com; path=/";
            }
            if (s.visitorMigrationKey || s_visitorMigrationKey) {
                return false;
            }
        }];
        u.send = function (a, b, c, d, e, f, g, h, ev, evt) {
            if (u.ev[a] || typeof u.ev.all != "undefined") {
                ...
                if (typeof b._cprod != "undefined" && b._cprod != "") {
                    c = [];
                    d = {};
                    ev = {};
                    for (e in utag.loader.GV(u.map)) {
                        if (typeof b[e] != "undefined" && typeof u.map[e] == "string" && u.map[e].indexOf("PRODUCTS_") > -1) {
                            ...
                            for (g = 0; g < f.length; g++) {
                                if (f[g].indexOf("PRODUCTS_evar") == 0) {
                                    d[f[g].substring(9)] = b[e].split(",")
                                } else if (f[g].indexOf("PRODUCTS_event") == 0) {
                                    ev[f[g].substring(9)] = b[e]
                                }
                            }
                        }
                    }
                    ...
                    for (d = 0; d < b._cprod.length; d++) {
                        var h2 = h;
                        if (typeof b.sc_prodevents[d] != "undefined") {
                            for (f in utag.loader.GV(b.sc_prodevents[d])) {
                                if (h2) h2 += "|" + f + '=' + b.sc_prodevents[d][f];
                                else h2 = f + '=' + b.sc_prodevents[d][f];
                            }
                        }
                ...
                evt = /^event|prodView|scOpen|scAdd|scRemove|scView|scCheckout|purchase$/;
                    ...
                    try {
                        ...
                        for (d in c) {
                            if (typeof c[d] != 'function') {
                                if (typeof t[d] != 'undefined' && t[d] != '' && t[d].toString().indexOf('D=') != 0) {
...
function sc_npv(v, va, d) {
    ...
    if (tD < cD || d == 0) {
        ...
        for (var i = 0; i < vAr.length; i++) {
            if (vAr[i] == 'prop70') {
                utag.data['action'] = vaAr[i];
            }
            s[vAr[i]] = vaAr[i];
            if (s[vAr[i]].match(/event/g)) {
                s.linkTrackEvents += vaAr[i] + ",";
                s.linkTrackVars += 'events,';
            } else {
                s.linkTrackVars += vAr[i] + ",";
            }
        ...
        utag.data['call_type'] = 'nonpage view';
        s.tl(this, 'o', 'customlink');
    }
}

function s_initial_beacon() {
    if ((document.location.host.indexOf('dna.ancestry') != -1) || (document.location.host.indexOf('ancestrydna') != -1)) {
        if (s.pageName) {
            pName = s.pageName;
        }
        if (pNameA = pName.split(' : ')) {
    ...
    if (typeof (dna_deny) !== 'undefined') {
        if (dna_deny === true && s.pageName == 'ancestry us : dna : membermatch : at member match') {
            s.pageName = 'ancestry us : dna : offer : deny : dna match';
            s.events = 'event3';
        }
    }
    s.t();
